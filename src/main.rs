...
fn main() -> Result<(), Error> {
        ...
        .enumerate()
    {
        ...
        let last_price = &ct["Last"].as_object().unwrap()["Price"]["Value"]
            .as_f64()
            .unwrap();
        let high = &ct["High"]["Value"].as_f64().unwrap();
        let low = &ct["Low"]["Value"].as_f64().unwrap();
        ...
        // Check if the last trade was within the last 10 minutes
        let trading = Utc::now() - utc_dt < chrono::Duration::minutes(10);
        ...
        // Duplicate the first row for the top row , but don't set the font
        if i == 0 {
            let mut top_row = row.clone();
            top_row.add_cell(cell!(format!("color={}", top_color)));
